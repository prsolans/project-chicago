...
export type PhraseCategory = 'medical' | 'comfort' | 'social' | 'responses' | 'questions' | 'family';
...
const SOCIAL_PHRASES: Record<TimeOfDay | 'anytime', StaticPhrase[]> = {
  ...
  night: [
    { content: 'Good night', confidence: 0.95, timeAware: true },
...
const FAMILY_PHRASES: Record<TimeOfDay | 'anytime', StaticPhrase[]> = {
  ...
  night: [
    { content: 'I love you Tony', confidence: 0.95, timeAware: true },
...
export function getStaticPhrases(
  ...
  const phraseSets: Record<PhraseCategory, Record<TimeOfDay | 'anytime', StaticPhrase[]>> = {
  ...
  const categoryPhrases = phraseSets[category];
